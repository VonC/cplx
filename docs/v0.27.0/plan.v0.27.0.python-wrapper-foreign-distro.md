...
### Why step 4 is split, and why the freeze keeps its number

Step 4 was one step until its own code review refused it as one. The cause is
decision Q08/H2 above: the manifest names the commit that FROZE the harness, and
a commit id cannot be written into a file that commit contains. The freeze must
therefore be committed before the manifest can be generated at all, and a step
whose criteria span two commits cannot be reviewed or gated as one unit.

The round 1 answer put it plainly: the reviewed identity is the whole step, its
criteria require the manifest, the pipeline copies, the build and the control in
...
was already open, and a review exchange pins its implementation step for the
...
- `docs/v0.27.0/verify.wrapper-scope.sh` (existing, FROZEN by this sub-step)
- `docs/v0.27.0/verify.wrapper-scope.step0.rhel.txt` (existing, retaken)
- `docs/v0.27.0/verify.wrapper-scope.step1.rhel.txt` (existing, retaken)
- `docs/v0.27.0/verify.wrapper-scope.step2.rhel.txt` (existing, retaken)
- `docs/v0.27.0/verify.wrapper.rhel.txt` (existing, retaken)

The four captures are named because a freeze that left them bound to earlier
bytes would freeze a harness no retained measurement describes.
...
Landed by this sub-step in the pipeline repository, by the cplx maintainer:
...
- THE MANIFEST, written in a commit AFTER the step 4 freeze commit, naming that
  ...
  bytes at that path. It never names its own commit, which is impossible;
