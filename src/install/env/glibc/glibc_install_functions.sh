...
function configure() {
    ...
    mkdir -p "${tool_src}/build" || fatal "Unable to create build directory in '$(pwd)'" 17
    cd "${tool_src}/build" || fatal "Unable to change to build directory in '$(pwd)'" 18
    rm -f "${tool_src}/config.log"
    ln -nfs build/config.log "${tool_src}/config.log"
